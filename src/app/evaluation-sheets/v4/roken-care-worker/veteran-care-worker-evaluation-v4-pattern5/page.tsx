...
type TechnicalScoreCategory = 'expertise' | 'leadership' | 'education' | 'development';
type EvaluatorType = 'superiorEval' | 'selfEval';

export default function RokenVeteranCareWorkerEvaluationV4Pattern5() {
  const [technicalScores, setTechnicalScores] = useState<{
    [key in EvaluatorType]: {
      [key in TechnicalScoreCategory]: number;
    };
  }>({
    superiorEval: { expertise: 0, leadership: 0, education: 0, development: 0 },
    selfEval: { expertise: 0, leadership: 0, education: 0, development: 0 }
  });
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-center">
            老健 ベテラン介護職員（11年以上） 人事評価シート v4.0
          </CardTitle>
          <p className="text-center text-gray-600 mt-2">
            介護老人保健施設におけるベテラン介護職員（無資格）の評価
          </p>
        </CardHeader>
        <CardContent>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input id="employeeCode" placeholder="例: CS004" />
                    </div>
                    <div>
                      <Label htmlFor="employeeName">氏名</Label>
                      <Input id="employeeName" placeholder="佐藤 洋子" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      ...
                      <Input id="position" placeholder="ベテラン介護職員" defaultValue="ベテラン介護職員" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="qualification">保有資格</Label>
                      <Input id="qualification" placeholder="介護職員実務者研修修了" />
                    </div>
                    <div>
                      <Label htmlFor="experience">介護経験</Label>
                      <Input id="experience" placeholder="13年2ヶ月" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <Tabs defaultValue="superior" className="w-full">
                      ...
                      <TabsContent value="superior" className="space-y-4">
                        <div className="space-y-4">
                          <div>
                            <Label className="text-base font-semibold">1. 高度な専門性・実践力</Label>
                            <p className="text-sm text-gray-600 mb-2">
                              熟練した介護技術、経験に基づく実践知識、困難事例への対応
                            </p>
                              ...
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    ...
                                    <Label htmlFor={`superior-expertise-${grade}`} className="ml-2">
                          ...
                          <div>
                            <Label className="text-base font-semibold">2. 組織的リーダーシップ</Label>
                            <p className="text-sm text-gray-600 mb-2">
                              介護職員チームの統括、業務管理、組織運営への参画
                            </p>
                              ...
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    ...
                                    <Label htmlFor={`superior-leadership-${grade}`} className="ml-2">
                                      {grade}({grade === 'S' ? '優秀' : grade === 'A' ? '良好' : grade === 'B' ? '標準' : grade === 'C' ? '要努力' : '要改善'})
                                    </Label>
                                  </div>
                                ))}
                          ...
                          <div>
                            <Label className="text-base font-semibold">3. 教育・指導力</Label>
                            <p className="text-sm text-gray-600 mb-2">
                              新人・後輩の系統的指導、技術伝承、研修企画・実施
                            </p>
                            <RadioGroup onValueChange={(value) => handleTechnicalScoreChange('superiorEval', 'education', value as keyof typeof gradeToScore)}>
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    <RadioGroupItem value={grade} id={`superior-education-${grade}`} />
                                    <Label htmlFor={`superior-education-${grade}`} className="ml-2">
                                      {grade}({grade === 'S' ? '優秀' : grade === 'A' ? '良好' : grade === 'B' ? '標準' : grade === 'C' ? '要努力' : '要改善'})
                                    </Label>
                                  </div>
                                ))}
                          ...
                          <div>
                            <Label className="text-base font-semibold">4. 資格取得・自己成長</Label>
                            <p className="text-sm text-gray-600 mb-2">
                              介護福祉士取得への取り組み、生涯学習、専門性向上への意識
                            </p>
                      ...
                      <TabsContent value="self" className="space-y-4">
                        <div className="space-y-4">
                              ...
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    ...
                                    <Label htmlFor={`self-expertise-${grade}`} className="ml-2">
                                      {grade}({grade === 'S' ? '優秀' : grade === 'A' ? '良好' : grade === 'B' ? '標準' : grade === 'C' ? '要努力' : '要改善'})
                                    </Label>
                                  </div>
                                ))}
                              ...
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    ...
                                    <Label htmlFor={`self-leadership-${grade}`} className="ml-2">
                                      {grade}({grade === 'S' ? '優秀' : grade === 'A' ? '良好' : grade === 'B' ? '標準' : grade === 'C' ? '要努力' : '要改善'})
                                    </Label>
                                  </div>
                                ))}
                          ...
                          <div>
                            ...
                            <RadioGroup onValueChange={(value) => handleTechnicalScoreChange('selfEval', 'education', value as keyof typeof gradeToScore)}>
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    <RadioGroupItem value={grade} id={`self-education-${grade}`} />
                                    <Label htmlFor={`self-education-${grade}`} className="ml-2">
                                      {grade}({grade === 'S' ? '優秀' : grade === 'A' ? '良好' : grade === 'B' ? '標準' : grade === 'C' ? '要努力' : '要改善'})
                                    </Label>
                                  </div>
                                ))}
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        この評価シートは、老健におけるベテラン介護職員の成長と貢献度を公正に評価し、
                        今後のキャリア開発を明確にすることを目的としています。
                      </p>
                    </div>
                    <div>
                      <h3 className="font-semibold mb-2">ベテラン介護職員の期待役割</h3>
                      <ul className="list-disc list-inside text-sm text-gray-600 space-y-1">
                        <li>長年の経験に基づく高度な介護実践</li>
                        <li>介護職員チーム全体の統括・リーダーシップ</li>
                        <li>後輩職員への系統的指導と技術伝承</li>
                        <li>組織運営への積極的参画</li>
                        <li>継続的な自己成長と専門性向上</li>
                      </ul>
                    </div>
                  </div>
                </CardContent>
