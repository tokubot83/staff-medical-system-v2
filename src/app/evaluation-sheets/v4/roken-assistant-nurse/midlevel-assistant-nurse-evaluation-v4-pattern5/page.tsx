...
type TechnicalScoreCategory = 'clinicalSkills' | 'leadership' | 'guidance' | 'innovation';
...
export default function RokenMidlevelAssistantNurseEvaluationV4Pattern5() {
  const [technicalScores, setTechnicalScores] = useState<{
    ...
    superiorEval: { clinicalSkills: 0, leadership: 0, guidance: 0, innovation: 0 },
    selfEval: { clinicalSkills: 0, leadership: 0, guidance: 0, innovation: 0 }
  });
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-center">
            老健 中堅准看護師（4-10年目） 人事評価シート v4.0
          </CardTitle>
          <p className="text-center text-gray-600 mt-2">
            介護老人保健施設における中堅准看護師の評価
          </p>
        </CardHeader>
        <CardContent>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input id="employeeCode" placeholder="例: LN003" />
                    </div>
                    <div>
                      <Label htmlFor="employeeName">氏名</Label>
                      <Input id="employeeName" placeholder="高橋 恵子" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      ...
                      <Input id="position" placeholder="中堅准看護師" defaultValue="中堅准看護師" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      ...
                      <Input id="licenseNumber" placeholder="第L-34567号" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <Tabs defaultValue="superior" className="w-full">
                      ...
                      <TabsContent value="superior" className="space-y-4">
                        <div className="space-y-4">
                          <div>
                            <Label className="text-base font-semibold">1. 高度な看護実践</Label>
                            <p className="text-sm text-gray-600 mb-2">
                              高度な技術提供、複雑な病態への対応、総合的な看護判断
                            </p>
                              ...
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    <RadioGroupItem value={grade} id={`superior-clinical-${grade}`} />
                                    <Label htmlFor={`superior-clinical-${grade}`} className="ml-2">
                          ...
                          <div>
                            <Label className="text-base font-semibold">2. チームリーダーシップ</Label>
                            <p className="text-sm text-gray-600 mb-2">
                              看護補助者への指導、業務調整、問題解決
                            </p>
                          ...
                          <div>
                            <Label className="text-base font-semibold">3. 後輩指導・人材育成</Label>
                            <p className="text-sm text-gray-600 mb-2">
                              新人・後輩の指導、技術指導、成長支援
                            </p>
                              ...
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    ...
                                    <Label htmlFor={`superior-guidance-${grade}`} className="ml-2">
                                      {grade}({grade === 'S' ? '優秀' : grade === 'A' ? '良好' : grade === 'B' ? '標準' : grade === 'C' ? '要努力' : '要改善'})
                                    </Label>
                                  </div>
                                ))}
                          ...
                          <div>
                            <Label className="text-base font-semibold">4. 業務改善・質向上</Label>
                            <p className="text-sm text-gray-600 mb-2">
                              業務効率化提案、看護の質向上活動、新しい取り組み
                            </p>
                          ...
                          <div>
                            ...
                            <RadioGroup onValueChange={(value) => handleTechnicalScoreChange('selfEval', 'clinicalSkills', value as keyof typeof gradeToScore)}>
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    <RadioGroupItem value={grade} id={`self-clinical-${grade}`} />
                                    <Label htmlFor={`self-clinical-${grade}`} className="ml-2">
                                      {grade}({grade === 'S' ? '優秀' : grade === 'A' ? '良好' : grade === 'B' ? '標準' : grade === 'C' ? '要努力' : '要改善'})
                                    </Label>
                                  </div>
                                ))}
                              ...
                              <div className="flex space-x-4">
                                {['S', 'A', 'B', 'C', 'D'].map(grade => (
                                  <div key={grade} className="flex items-center">
                                    ...
                                    <Label htmlFor={`self-guidance-${grade}`} className="ml-2">
                                      {grade}({grade === 'S' ? '優秀' : grade === 'A' ? '良好' : grade === 'B' ? '標準' : grade === 'C' ? '要努力' : '要改善'})
                                    </Label>
                                  </div>
                                ))}
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        この評価シートは、老健における中堅准看護師の成長と貢献度を公正に評価し、
                        今後のキャリア開発を明確にすることを目的としています。
                      </p>
                    </div>
                    <div>
                      <h3 className="font-semibold mb-2">中堅准看護師の期待役割</h3>
                      <ul className="list-disc list-inside text-sm text-gray-600 space-y-1">
                        <li>高度な看護技術の習得と実践</li>
                        <li>チーム内でのリーダーシップ発揮</li>
                        <li>後輩・新人への積極的な指導</li>
                        <li>業務改善と看護の質向上への貢献</li>
                        <li>多職種連携における中核的役割</li>
                      </ul>
                    </div>
                  </div>
                </CardContent>
